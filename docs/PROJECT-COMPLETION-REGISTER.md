...
| Administration | One administration surface for customers, KYC/AML, transactions, content, email, security, integrations, reports and configuration. | Central authentication/authorization, CSRF protection, audit logging, KYC/KYB cases, signed-provider onboarding and ongoing sanctions/PEP/adverse-media rescreening queue, operations inbox and sponsor-readiness workspace. | Operational control plane implemented; contracted provider certification and periodic RBAC review remain external work. |
...
- Added a PostgreSQL-backed independent-assurance register for penetration tests, isolated disaster-recovery exercises and compliance acceptance, with controlled evidence hashes, finding counts, role ownership, maker-checker review and immutable history. Exercise acceptance cannot enable real funds.
