...
export async function createCustomerSession(
  ...
  // When DB is not configured, the token is stored on the user record
  // by the flat-file userStore (backward compat).
...
/**
 * List active sessions for a user (for the "manage devices" screen).
 * Returns [] when DB isn't configured — flat-file mode stores at most one
 * implicit session on the user record itself, nothing to enumerate.
 */
export async function listCustomerSessions(userId: string): Promise<Array<{
  token: string; ip: string | null; ua: string | null; createdAt: string; lastSeenAt: string; expiresAt: string;
}>> {
  if (!isDatabaseConfigured()) return [];
  const db = getDb();
