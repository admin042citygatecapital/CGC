...
/** Paginated audit log read — newest first. */
export function readAudit(limit = 200, offset = 0): { data: AuditEntry[]; total: number } {
  try {
    if (!fs.existsSync(AUDIT_FILE)) return { data: [], total: 0 };
