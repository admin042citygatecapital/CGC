...
export function addSubscriber(email: string, name?: string, source?: string): Subscriber {
  ...
  const sub: Subscriber = { id: 'sub_' + crypto.randomBytes(8).toString('hex'), email: email.toLowerCase(), name, status: 'active', source, subscribedAt: new Date().toISOString(), sequenceStep: 0 };
  subs.push(sub);
  saveAll(subs);
  return sub;
}

export function updateSequenceStep(email: string, step: number): void {
  const subs = loadAll();
  const idx = subs.findIndex(s => s.email.toLowerCase() === email.toLowerCase());
  if (idx === -1) return;
  subs[idx].sequenceStep = step;
  subs[idx].lastEmailAt = new Date().toISOString();
  saveAll(subs);
}
