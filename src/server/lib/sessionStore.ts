...
export async function createSession(token: string, data: Session): Promise<void> {
...
export async function getSession(token: string, fingerprint?: { ip: string; ua: string }): Promise<Session | null> {
  if (!token || token.length !== 64) return null;
  if (!isDatabaseConfigured()) return (await ff()).getSession(token, fingerprint);
  const db  = getDb();
  const now = new Date();
  ...
  // IP + UA fingerprint check — deny this request on mismatch (a stolen
  // token used from a different device/network fails here) without
  // deleting the underlying session, so the legitimate device's own
