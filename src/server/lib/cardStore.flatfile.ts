...
import type { VirtualCard, CardActivity, CardPatch } from './cardStore.js';
...
export function updateCard(id: string, patch: CardPatch): VirtualCard | null {
  const cardsData = loadAll();
  const idx = cardsData.findIndex(c => c.id === id);
  if (idx === -1) return null;
  // Normalize explicit `null` (clear field) to `undefined` for the flat-file
  // VirtualCard shape, which only ever stores number/string or omits the key.
  const normalized: Partial<VirtualCard> = {
    ...patch,
    spendingLimit: patch.spendingLimit === null ? undefined : patch.spendingLimit,
    pinHash:       patch.pinHash       === null ? undefined : patch.pinHash,
  };
  cardsData[idx] = { ...cardsData[idx], ...normalized, updatedAt: new Date().toISOString() };
  saveAll(cardsData);
  return cardsData[idx];
}

// FIX (Category C, task #38): ownership check added — see cardStore.ts deleteCard for full rationale.
export function deleteCard(id: string, userId: string): boolean {
  const cardsData = loadAll();
  const idx = cardsData.findIndex(c => c.id === id && c.userId === userId);
