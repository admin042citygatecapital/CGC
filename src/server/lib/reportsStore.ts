...
import { getAllExpiry, getExpiringWithin } from './kycExpiryStore.js';
...
export async function kycReport(q: ReportQuery) {
  ...
  // Expiring soon / expired (from the dedicated KYC expiry tracker)
  const allExpiry = getAllExpiry();
  const expiringSoon = getExpiringWithin(30).length;
  const expired = allExpiry.filter(r => r.status === 'expired').length;
