...
export async function requireCustomerAuth(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  // FIX (see AUDIT_REPORT.md §7): this was previously called without `await`.
  // Since it's async, the returned Promise object was always truthy, so the
  // `if (!user)` rejection path below could never trigger — every request
  // with a syntactically-shaped-but-invalid token was treated as authenticated.
