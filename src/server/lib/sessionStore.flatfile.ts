...
export function createSession(token: string, data: Session): void {
  const sessions = load();
...
export function getSession(token: string, fingerprint?: { ip: string; ua: string }): Session | null {
  ...
  // IP + UA fingerprint check — deny this request without deleting the
  // session (see sessionStore.ts's getSession for the full rationale).
