...
/** Env-configured key (SMARTSUPP_KEY / SMARTSUPP_API_KEY) used when no admin-set apiKey is persisted. */
function envApiKey(): string {
  return String(getSecret('SMARTSUPP_KEY') || getSecret('SMARTSUPP_API_KEY') || '').trim();
}

export function readConfig(): SmartsuppConfig {
  try {
    ensureDir();
    const base = { ...DEFAULT_CONFIG, apiKey: envApiKey() || DEFAULT_CONFIG.apiKey };
    if (!fs.existsSync(FILES.config)) return base;
    const persisted = JSON.parse(fs.readFileSync(FILES.config, 'utf8')) as Partial<SmartsuppConfig>;
    return { ...base, ...persisted, apiKey: persisted.apiKey || base.apiKey };
  } catch { return { ...DEFAULT_CONFIG, apiKey: envApiKey() || DEFAULT_CONFIG.apiKey }; }
}
