...
function readAllAlerts(): PriceAlert[] {
  try {
    if (!fs.existsSync(ALERTS_FILE)) return [];
    const lines = fs.readFileSync(ALERTS_FILE, 'utf8').trim().split('\n').filter(Boolean);
    return lines.map(l => { try { return JSON.parse(l) as PriceAlert; } catch { return null; } })
      .filter((a): a is PriceAlert => a !== null);
  } catch { return []; }
}

function writeAllAlerts(alerts: PriceAlert[]): void {
  ensureAlertsDir();
  fs.writeFileSync(ALERTS_FILE, alerts.map(a => JSON.stringify(a)).join('\n') + (alerts.length ? '\n' : ''), 'utf8');
}

export function getAlerts(userId: string): PriceAlert[] {
  return readAllAlerts().filter(a => a.userId === userId);
}

export function createAlert(data: Omit<PriceAlert, 'id' | 'createdAt' | 'active'>): PriceAlert {
  const alerts = readAllAlerts();
  const alert: PriceAlert = { ...data, id: randomUUID(), active: true, createdAt: new Date().toISOString() };
  alerts.push(alert);
  writeAllAlerts(alerts);
  return alert;
}
