...
function readLatest(): OperationsItem[] {
  ...
  for (const line of fs.readFileSync(FILE, 'utf8').split('\n').filter(Boolean)) {
    ...
    } catch { /* preserve the rest of the append-only log */ }
  }
  return [...byId.values()];
}
...
export function createOperationsItem(input: CreateOperationsInput, skipDeduplication = false): OperationsItem {
...
export function updateOperationsItem(id: string, changes: OperationsUpdate, actor: string): OperationsItem | null {
...
export function getAllOperationsItems(): OperationsItem[] {
  return readLatest();
}
