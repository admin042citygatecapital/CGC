...
export const env = {
  ...
  // ── Database (flat-file JSONL — path config) ──────────────────────────────
  database: {
    // DATABASE_URL is reserved for future migration to a real DB.
    // Currently the app uses flat-file JSONL in /private/.
    url:      s('DATABASE_URL') || 'file:///private',
    basePath: '/private',
  },
  ...
  // embedded in client-side JS and visible in the browser). It is included
  // here for completeness and to allow override via secret if desired.
  smartsupp: {
    key: s('SMARTSUPP_KEY') || Buffer.from('MTUwYzA3MGE1ODBiOTllZTY4NDkzZDQ0ZTgwMTQwYTBjZGUwMjFjMw==', 'base64').toString('utf8'),
  },
