/**
 * smtpTransport.ts — Resend HTTP transport (Workers-compatible)
 * ──────────────────────────────────────────────────────────────
 * Replaces Nodemailer + Zoho SMTP with Resend's HTTPS REST API.
 * Resend uses plain fetch() — no TCP sockets, no Node.js net/tls.
 *
 * Exported interface is identical to the previous implementation so
...
function resolveFrom(): string {
  return (
    getSecret('MAIL_FROM_ADDRESS') ||
    'noreply@citygate.capital'
  );
}
