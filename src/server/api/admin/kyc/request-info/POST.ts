...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    const result = await decideKycCase({ caseId: record.id, decision: 'needs_info', reason: String(req.body?.message ?? req.body?.reason ?? ''), reasonCode: String(req.body?.reasonCode ?? 'other'), requestedEvidenceKinds: Array.isArray(req.body?.requestedEvidenceKinds) ? req.body.requestedEvidenceKinds : [], expectedVersion: Number(req.body?.expectedVersion), adminId: session.adminId, adminEmail: session.email, adminRole: session.role, ip: req.ip, requestId: String(req.headers['x-request-id'] ?? '') });
