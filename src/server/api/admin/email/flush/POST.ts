...
export default async function handler(req: Request, res: Response) {
  ...
  // ── Step 3: Auto-requeue exhausted emails ────────────────────────────────────
  ...
  if (requeuedIds.length > 0) {
    console.log(JSON.stringify({ event: 'email.flush.requeued_exhausted', count: requeuedIds.length }));
  }
  ...
  for (const email of pending) {
    ...
    await updateQueuedEmail(email.id, { status: 'retrying', attempts: email.attempts + 1, lastAttemptAt: now });

    try {
      const result = await smtpSendEmail({ to: email.to, subject: email.subject, html: email.html });

      if (result.success) {
        await markEmailSent(email.id);
        flushResults.push({ id: email.id, to: email.to, subject: email.subject, success: true, transport: result.transport });
      } else {
        ...
        flushResults.push({ id: email.id, to: email.to, subject: email.subject, success: false, transport: result.transport, error: result.error });
      }
    } catch (err) {
      ...
      flushResults.push({ id: email.id, to: email.to, subject: email.subject, success: false, error: msg });
    }
  }
