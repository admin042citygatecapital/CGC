 ...
 * access + refresh tokens without requiring the in-memory CSRF state check.
...
export default async function handler(req: Request, res: Response) {
    ...
    if (!code || typeof code !== 'string' || !code.startsWith('1000.')) {
      return res.status(400).json({ error: 'Missing or invalid code. Must start with 1000.' });
    }
    ...
    if (!clientId || !clientSecret) {
      return res.status(503).json({ error: 'ZOHO_CLIENT_ID or ZOHO_CLIENT_SECRET not configured in secrets.' });
    }
    ...
    for (const tokenUrl of ZOHO_TOKEN_URLS) {
      ...
      try {
        ...
        lastError = String(data.error ?? data.error_description ?? lastStatus);
      } catch (err) {
    ...
    return res.status(502).json({
      ...
      hint:  lastError === 'invalid_code'
        ? 'The authorization code has expired (60s TTL) or was already used. Re-run the OAuth flow at /api/zoho/connect.'
