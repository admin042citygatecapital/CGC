 ...
 * Revoke a specific trusted device or all devices for the authenticated admin.
 */
...
export default async function handler(req: Request, res: Response) {
  ...
  if (token) {
    // Verify the token belongs to this admin before revoking
    ...
    if (!match) return res.status(404).json({ ok: false, error: 'Device not found' });
  ...
  return res.status(400).json({ ok: false, error: 'Provide token or all:true' });
}
