...
import { reviewLegalEntity, saveLegalEntity, submitLegalEntity } from '../../../lib/legalEntityVerificationStore.js';
...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    if (action === 'save') return res.json({ ok: true, data: await saveLegalEntity(req.body, actor) });
    if (action === 'submit') return res.json({ ok: true, data: await submitLegalEntity(actor) });
    if (action === 'review') {
      const decision = String(req.body?.decision ?? '');
      if (!['verified','rejected'].includes(decision)) return res.status(400).json({ error: 'Decision must be verified or rejected.' });
      return res.json({ ok: true, data: await reviewLegalEntity(decision as 'verified'|'rejected', String(req.body?.note ?? ''), actor) });
    }
