/** POST /api/admin/cards/issue — disabled until an approved issuer adapter exists. */
...
export default function handler(req: Request, res: Response) {
  ...
  if (!LIVE_CARD_ISSUER_ADAPTER_IMPLEMENTED) {
    return res.status(501).json({
      error: 'Card issuing is unavailable: no card issuer-processor is contracted for this platform, so card records remain read-only.',
