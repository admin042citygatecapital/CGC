/** POST /api/admin/cards/replace — disabled until an approved issuer adapter exists. */
...
export default function handler(req: Request, res: Response) {
  ...
  if (!LIVE_CARD_ISSUER_ADAPTER_IMPLEMENTED) {
    return res.status(501).json({
      error: 'Card replacement is unavailable: no card issuer-processor is contracted for this platform, so card records remain read-only.',
      code: 'CARD_PROVIDER_NOT_CONFIGURED',
    });
  }
  if (!requireCardOperations(res)) return;
  return res.status(501).json({ error: 'Card replacement route is not implemented', code: 'CARD_ISSUER_ROUTE_NOT_IMPLEMENTED' });
}
