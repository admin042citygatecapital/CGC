...
export default function handler(req: Request, res: Response) {
  ...
  try {
    if (!existsSync(REPLIES_FILE)) return res.json({ ok: true, replies: [], total: 0 });
    ...
    return res.json({ ok: true, replies, total: replies.length });
  } catch (err) {
    console.error('admin.tickets.replies.get.error', err);
    return res.status(500).json({ ok: false, error: 'Failed to load replies.' });
  }
}
