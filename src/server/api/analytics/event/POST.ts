...
export default function handler(req: Request, res: Response) {
  try {
    ...
    if (!page || typeof page !== 'string') {
      return res.status(400).json({ ok: false, error: 'page is required' });
    }
    ...
    return res.status(500).json({ ok: false, error: 'Failed to record event' });
  }
}
