...
import {
  X, User, Camera, Shield, FileText, CheckCircle,
  ArrowRight, ArrowLeft, Eye, EyeOff, Upload,
  Loader2, Building2, TrendingUp, Mail, Phone,
  Lock, Globe, CreditCard, AlertCircle,
} from 'lucide-react';
...
function StepIdentity({ data, onChange, onFile, errors }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Upload box */}
      <div>
        <Label>Upload Passport or Driver's License *</Label>
        <button
          type="button"
...
function StepConfirm({ data, onChange, onSendOTP, otpSent, errors }: {
  data: FormData;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-primary/10 bg-white/[0.02] p-5">
        ...
        <p className="text-xs text-foreground/40 mb-4">We'll send a 6-digit code to <span className="text-primary">{data.email || 'your email'}</span></p>
        <div className="flex gap-3">
          <Input
            ...
            maxLength={6}
            className="flex-1 tracking-[0.3em] text-center font-mono"
          />
          <button
            type="button"
            onClick={onSendOTP}
            className="relative px-4 py-3 rounded-xl text-sm font-bold text-black overflow-hidden shrink-0"
          >
            <div className="absolute inset-0 bg-gradient-to-r from-primary to-[#F0D080]" />
            <span className="relative">{otpSent ? 'Resend' : 'Send OTP'}</span>
          </button>
        </div>
        {otpSent && <p className="text-xs text-emerald-400 mt-2 flex items-center gap-1"><CheckCircle size={10} /> OTP sent — check your inbox</p>}
...
export default function AccountOpeningModal({ open, onClose, initialPlan = 'Personal' }: AccountOpeningModalProps) {
  ...
  function validate(): boolean {
    ...
    if (step === 1) {
      ...
      if (!form.idFile)             errs.idFile      = 'Please upload your ID document' as never;
    }
    if (step === 2) {
    ...
    if (step === 3) {
      if (!form.otp.trim() || form.otp.length < 4) errs.otp = 'Enter the OTP sent to your email';
  ...
  function handleSendOTP() {
    setOtpSent(true);
    // In production: POST /api/newsletter/otp or similar
  }
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      await fetch('/api/accounts/apply', {
        method: 'POST',
      ...
      // Still show success — backend is best-effort for demo
      setSubmitted(true);
    } finally {
      setSubmitting(false);
    }
        ...
        <>
          ...
          >
            ...
            >
                ...
                {submitted ? (
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {step === 3 && <StepConfirm data={form} onChange={(k, v) => update(k, v)} onSendOTP={handleSendOTP} otpSent={otpSent} errors={errors} />}
                    </motion.div>
                  </AnimatePresence>
                )}
              </div>
              ...
              {/* Footer actions */}
              {!submitted && (
                <div className="px-7 py-5 border-t border-primary/8 flex items-center gap-3 shrink-0">
                  {step > 0 && (
                    <button
                      onClick={handleBack}
