...
const MAX_MESSAGES = 24; // server rejects longer histories — trim before posting
...
export default function AriaChatPanel() {
  ...
  const send = useCallback(async () => {
    ...
    setError(null);
    setDraft('');
    // Keep the posted history within the server's budget: trimming here (rather
    // than failing) prevents a permanent dead end once the cap is reached.
    const history = [...messages, { role: 'user' as const, content: text }].slice(-MAX_MESSAGES);
    ...
    try {
      ...
      // Flush any buffered tail of a split multi-byte character.
      streamed += decoder.decode();

      // A stream that ends with zero text is a provider failure that surfaced
      // after the 200 (see the route's onError note) — treat it as an error
      // instead of rendering a silently empty reply.
      if (!streamed) {
        throw new Error('The assistant returned no response. Please try again.');
      }
    ...
    >
      ...
      >
        <textarea
          ...
          onKeyDown={(event) => {
            // isComposing guards IME composition (CJK keyboards): Enter confirms
            // a candidate and must not send the half-composed text.
