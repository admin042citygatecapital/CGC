...
describe('bruteForce', () => {
  it('records failed attempts', async () => {
    ...
    const count = getFailCount(email);
  ...
  it('locks out after max attempts', async () => {
    ...
    const status = checkLockout(email, ip);
  ...
  it('clears lockout on success', async () => {
    ...
    expect(checkLockout(email, ip).blocked).toBe(true);
    recordLoginSuccess(email, ip);
    expect(checkLockout(email, ip).blocked).toBe(false);
  });
});
