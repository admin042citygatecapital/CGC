...
describe('customer rewards boundary',()=>{
  it('is controlled by the shared administrator feature control',()=>{const config=read('src/server/lib/configStore.ts');const admin=read('src/pages/admin/config.tsx');const api=read('src/server/api/users/rewards/GET.ts');expect(config).toContain('platformFeatures');expect(admin).toContain("['rewards','Rewards']");expect(api).toContain("isPlatformFeatureEnabled('rewards')");});
