...
describe('operations inbox store', () => {
  it('creates, deduplicates, filters, and updates submissions with history', async () => {
    ...
    const duplicate = await store.createOperationsItem({
      source: 'contact_form', referenceId: 'contact-1', title: 'Duplicate', summary: 'Duplicate',
    });
    expect(duplicate.id).toBe(created.id);
    expect((await store.listOperationsItems({ search: 'customer@example.test' })).total).toBe(1);

    const updated = await store.updateOperationsItem(created.id, {
    ...
    expect(await store.getOperationsStats()).toMatchObject({ total: 1, open: 1, urgent: 1, inReview: 1 });
  });
  ...
  it('backfills a legacy application once without copying sensitive identity fields', async () => {
    ...
    const store = await import('../../server/lib/operationsInboxStore.js');
    await store.syncLegacyOperationsItems();
    await store.syncLegacyOperationsItems();
    const result = await store.listOperationsItems({ source: 'account_application' });
