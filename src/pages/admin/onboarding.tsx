...
type CaseRow = { id: string; userId: string; caseType: 'individual' | 'business'; status: Status; version: number; submittedBy?: string; lastEditedBy: string; updatedAt: string; queuePosition?: number|null };
...
export default function AdminOnboardingPage() {
  ...
  return <AdminLayout>
    ...
    <div className="p-6 space-y-6">
      ...
      <section className="rounded-2xl border border-primary/15 bg-primary/[0.025] p-4"><div className="flex items-center justify-between"><div><h2 className="font-semibold">Registration queue</h2><p className="mt-1 text-xs text-foreground/45">Server-authoritative customer applications ordered by submission time.</p></div><span className="rounded-full bg-primary/10 px-3 py-1 text-xs text-primary">{cases.filter(item=>item.queuePosition).length} queued</span></div></section>
      <div className="grid lg:grid-cols-[1fr_1.4fr] gap-5">
        <div className="rounded-2xl border border-white/10 overflow-hidden">
          {loading ? <div className="p-8 flex justify-center"><Loader2 className="animate-spin"/></div> : cases.length === 0 ? <div className="p-8 text-sm text-foreground/45">No onboarding cases yet.</div> : cases.map(item => <button key={item.id} onClick={() => void openCase(item.id)} className="w-full text-left p-4 border-b border-white/8 hover:bg-white/5">
            <div className="flex justify-between gap-3"><span className="font-semibold">{item.userId}</span><span className="text-xs uppercase text-primary">{item.queuePosition ? `Queue #${item.queuePosition} · ` : ''}{item.status.replace('_',' ')}</span></div>
