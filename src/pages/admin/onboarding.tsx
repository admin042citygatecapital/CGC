...
type Bundle = { case: CaseRow & { requestedEvidenceKinds?: string[]; customerInstructions?: string|null }; evidence: Array<{ id: string; kind: string; referenceType: string; sha256?: string }>; documents:Array<{id:string;kind:string;mimeType:string;byteSize:number;state:string;version:number;originalName:string;createdAt:string}>; profile:Record<string,unknown>|null; events: Array<{ id: string; action: string; actorId: string; createdAt: string; fromStatus?: string; toStatus?: string }>; providerVerifications: { events: ProviderEvent[]; checks: { identityAccepted: boolean; kybAccepted: boolean; screeningClear: boolean } }; customer: CustomerDecision | null };
...
export default function AdminOnboardingPage() {
  ...
  const [requestedKinds, setRequestedKinds] = useState<string[]>(['additional']);
      ...
      <div className="grid lg:grid-cols-[1fr_1.4fr] gap-5">
        ...
        <div className="rounded-2xl border border-white/10 p-5">
          {!selected ? <div className="h-full min-h-64 flex items-center justify-center text-foreground/40"><FileCheck2 className="mr-2"/> Select a case</div> : <div className="space-y-5">
            ...
            <section><h3 className="text-xs font-bold uppercase text-foreground/50 mb-2">Identity profile</h3>{selected.profile ? <div className="grid grid-cols-2 gap-2 rounded-xl bg-white/5 p-3 text-xs">{Object.entries(selected.profile).filter(([key])=>!['documentNumber'].includes(key)).map(([key,value])=><div key={key}><span className="text-foreground/40">{key.replaceAll(/([A-Z])/g,' $1')}: </span>{String(value ?? '—')}</div>)}</div> : <p className="text-xs text-foreground/40">No profile submitted.</p>}</section>
            <section><h3 className="text-xs font-bold uppercase text-foreground/50 mb-2">Private evidence</h3>{selected.documents.map(document => <div key={document.id} className="mb-2 flex items-center justify-between rounded-xl bg-white/5 p-3 text-sm"><div><div className="font-semibold">{document.kind.replaceAll('_',' ')} · version {document.version}</div><div className="text-xs text-foreground/45">{document.originalName} · {Math.ceil(document.byteSize/1024)} KB · {document.state}</div></div><a href={`/api/admin/kyc/document?documentId=${encodeURIComponent(document.id)}`} target="_blank" rel="noreferrer" className="rounded-lg border border-white/10 px-3 py-2 text-xs">View securely</a></div>)}</section>
