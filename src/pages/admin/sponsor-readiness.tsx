...
export default function SponsorReadinessPage() {
  ...
  return <><Helmet><title>Sponsor Readiness — City Gate Capital Admin</title><meta name="robots" content="noindex, nofollow" /></Helmet><AdminLayout title="Sponsor Readiness"><main className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
    ...
    {loading && !data ? <div className="py-16 text-center text-white/40"><Loader2 className="w-6 h-6 animate-spin mx-auto mb-2" />Loading controls…</div> : data && <>
      ...
      <section className={`rounded-2xl border p-5 ${data.independentReviewer.configured ? 'border-emerald-400/15 bg-emerald-400/[0.035]' : 'border-amber-400/20 bg-amber-400/[0.045]'}`}><div className="flex items-start gap-3"><ShieldCheck className={`w-5 h-5 shrink-0 ${data.independentReviewer.configured ? 'text-emerald-300' : 'text-amber-300'}`} /><div><h2 className="text-white font-semibold">Independent review channel</h2><p className="text-white/45 text-xs mt-1">{data.independentReviewer.configured ? 'A separately authenticated checker identity and hashed credential are configured. This credential cannot create an administration session.' : 'Independent review is unavailable. Configure both the checker identity and the SHA-256 credential hash before submitted evidence can be reviewed.'}</p>{!data.independentReviewer.configured && <p className="text-amber-200/70 text-xs mt-2">Checker identity: {data.independentReviewer.emailConfigured ? 'configured' : 'missing'} · Credential hash: {data.independentReviewer.credentialHashConfigured ? 'configured' : 'missing'}</p>}</div></div></section>
