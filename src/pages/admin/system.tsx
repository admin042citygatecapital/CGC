...
export default function AdminSystem() {
  ...
  const status=(state:StatusState, healthyLabel='Operational') => {
    const healthy = state === 'healthy';
    const neutral = state === 'warning' || state === 'not_configured';
    const label = healthy ? healthyLabel : state === 'not_configured' ? 'Not configured' : state === 'warning' ? 'Monitoring' : 'Attention required';
    return <span className={`flex items-center gap-1 text-xs ${healthy?'text-emerald-300':neutral?'text-amber-300':'text-red-300'}`}>{healthy?<CheckCircle2 size={13}/>:<AlertTriangle size={13}/>} {label}</span>;
  };
  return <AdminLayout title="System Operations">
    ...
    <div className="space-y-6"><div className="flex items-start justify-between gap-4"><div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><Activity className="text-amber-300"/>System Operations</h1><p className="text-white/40 text-sm mt-1">Read-only service diagnostics and controlled operational actions. No shell or raw secrets are exposed.</p></div><button onClick={refresh} className="rounded-xl border border-white/10 p-2.5 text-white/40 hover:text-white"><RefreshCw size={16} className={loading?'animate-spin':''}/></button></div>
      <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3">
        <div className={card}><Database className="text-amber-300 mb-3" size={18}/><p className="text-white font-medium">PostgreSQL</p><p className="text-white/35 text-xs mt-1">Latency {health?.database?.latencyMs??'—'} ms</p><div className="mt-3">{status(health?.components?.database?.state)}</div></div>
        <div className={card}><Mail className="text-amber-300 mb-3" size={18}/><p className="text-white font-medium">Email delivery</p><p className="text-white/35 text-xs mt-1">{health?.email?.provider??'Unavailable'}</p><div className="mt-3">{status(health?.components?.email?.state)}</div></div>
        <div className={card}><Server className="text-amber-300 mb-3" size={18}/><p className="text-white font-medium">Application runtime</p><p className="text-white/35 text-xs mt-1">{health?.runtime?.nodeVersion??'—'} · {health?.uptime?.human??'—'}</p><div className="mt-3">{status(health?.components?.memory?.state)}</div></div>
        <div className={card}><Shield className="text-amber-300 mb-3" size={18}/><p className="text-white font-medium">Protected configuration</p><p className="text-white/35 text-xs mt-1">{health?.configuration?.optionalWarnings ? `${health.configuration.optionalWarnings} optional setting${health.configuration.optionalWarnings===1?'':'s'} incomplete` : 'Validated privately at startup'}</p><div className="mt-3">{status(health?.components?.configuration?.state,'Core configuration valid')}</div></div>
      </div>
      <div className="grid lg:grid-cols-2 gap-4">
        <section className={card}><h2 className="text-white font-semibold">Runtime details</h2><div className="mt-4 space-y-2 text-sm text-white/45">{[['Environment',health?.environment],['Active admin sessions',health?.runtime?.activeAdminSessions],['Active customer sessions',health?.runtime?.activeCustomerSessions],['Heap utilization',health?.memory?.heapUsagePct == null?'—':`${health.memory.heapUsagePct}% (${health.memory.heapUsedMb} MB / ${health.memory.heapLimitMb} MB limit)`],['Independent Sponsor Review',health?.configuration?.sponsorReview==='healthy'?'Configured':health?.configuration?.sponsorReview==='not_configured'?'Not configured':'Attention required'],['Tracked records',health?.storage?.trackedRecords]].map(([k,v])=><div key={String(k)} className="flex justify-between border-b border-white/5 pb-2"><span>{k}</span><span className="text-white/70">{String(v??'—')}</span></div>)}</div></section>
