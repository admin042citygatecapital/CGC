...
import { MapPin, Phone, Mail, Clock, Send, CheckCircle, MessageCircle, ArrowRight, Linkedin, Twitter } from 'lucide-react';
...
const contactChannels = [
  ...
  { icon: Twitter,       title: 'Twitter / X',   detail: '@CityGateCapital',             color: '#1DA1F2', action: 'Follow Us',   href: 'https://twitter.com/CityGateCapital'                       },
  { icon: Linkedin,      title: 'LinkedIn',      detail: 'City Gate Capital',            color: '#0A66C2', action: 'Connect',     href: 'https://linkedin.com/company/citygate-capital'             },
];
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:image" content={(() => { const u = new URL('/api/og', 'https://citygate.capital'); u.searchParams.set('title', 'Contact Us'); u.searchParams.set('description', 'Get in Touch with City Gate Capital'); return u.href; })()} />
        ...
        <meta name="twitter:image" content={(() => { const u = new URL('/api/og', 'https://citygate.capital'); u.searchParams.set('title', 'Contact Us'); u.searchParams.set('description', 'Get in Touch with City Gate Capital'); return u.href; })()} />
            ...
            {contactChannels.map((c, i) => (
              <motion.a
                ...
                <div>
                  ...
                  <p className="text-[10px] text-foreground/35">{c.detail}</p>
                </div>
              </motion.a>
            ))}
          </div>
              ...
              <div className="space-y-4 mb-12">
                {[
                  ...
                  <div key={item.title} className="flex items-start gap-4 p-4 glass rounded-2xl border border-primary/10 hover:border-primary/20 transition-colors group">
                    ...
                    <div>
                      <p className="text-xs text-foreground/40 uppercase tracking-wide mb-0.5">{item.title}</p>
                      {item.href ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {offices.map((office) => (
                  <div key={office.city} className={`glass-card rounded-xl p-3.5 transition-colors ${office.hq ? 'border border-primary/30' : 'gradient-border hover:border-primary/20'}`}>
                    ...
                    <p className="text-xs text-foreground/40 leading-relaxed">{office.address}</p>
                  </div>
                ))}
              </div>
            </motion.div>
              ...
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="glass-card rounded-3xl p-8 gradient-border space-y-5" style={{ boxShadow: 'var(--gold-glow)' }}>
                  <div>
                    ...
                    <p className="text-xs text-foreground/40">We respond within 2 business hours.</p>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={field.key}>
                        <label htmlFor={`contact-${field.key}`} className="text-xs text-foreground/40 uppercase tracking-wide mb-2 block">{field.label}</label>
                        <input
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="contact-email" className="text-xs text-foreground/40 uppercase tracking-wide mb-2 block">Email</label>
                      <input
                        id="contact-email"
                        required
                        type="email"
                    ...
                    <div>
                      <label htmlFor="contact-company" className="text-xs text-foreground/40 uppercase tracking-wide mb-2 block">Company <span className="text-foreground/20">(optional)</span></label>
                      <input
                        id="contact-company"
                        value={form.company}
                  ...
                  <div>
                    <label htmlFor="contact-subject" className="text-xs text-foreground/40 uppercase tracking-wide mb-2 block">Subject</label>
                    <select
                      id="contact-subject"
                      value={form.subject}
                  ...
                  <div>
                    <label htmlFor="contact-message" className="text-xs text-foreground/40 uppercase tracking-wide mb-2 block">Message</label>
                    <textarea
                      id="contact-message"
                      required
                      rows={5}
