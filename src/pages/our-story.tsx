...
import { ArrowRight, CheckCircle2, Landmark, MapPin, Scale, ShieldCheck } from 'lucide-react';
...
export default function OurStoryPage() {
  ...
  return <>
    ...
    <main className="overflow-hidden">
      ...
      <section className="py-24 border-y border-primary/10 bg-primary/[0.03]"><div className="container mx-auto px-4 md:px-6 max-w-4xl text-center"><Reveal><blockquote className="text-3xl md:text-5xl font-medium leading-tight">“Build the gate carefully. Explain what lies beyond it. Open it only when trust can travel both ways.”</blockquote><p className="text-primary mt-7 text-sm uppercase tracking-[0.2em]">City Gate Capital</p><address className="mt-4 text-sm leading-6 text-foreground/55 not-italic">51 Mosley Street<br />Manchester M2 3HQ<br />United Kingdom</address><div className="mt-8 overflow-hidden rounded-2xl border border-primary/20 bg-black/30 shadow-2xl"><iframe src={storyLocation.mapEmbedUrl} title="Google Map showing City Gate Capital at 51 Mosley Street, Manchester" className="h-72 md:h-96 w-full" loading="lazy" referrerPolicy="no-referrer-when-downgrade" allowFullScreen /></div><a href={storyLocation.directionsUrl} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-2 text-sm font-medium text-primary hover:text-primary/80"><MapPin size={17} /> Open in Google Maps</a><div><Link to="/contact" className="inline-flex items-center gap-2 mt-10 px-7 py-4 rounded-xl bg-primary text-black font-semibold">Talk to our team <ArrowRight size={18}/></Link></div></Reveal></div></section>
