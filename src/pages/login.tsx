...
export default function LoginPage() {
  ...
  const [notice, setNotice] = useState<{ msg: string; kind: 'warn' | 'info' } | null>(null);
  ...
  useEffect(() => {
    ...
    if (reason === 'session_expired') setNotice({ msg: 'Your session expired. Please log in again.', kind: 'warn' });
    else if (reason === 'auth_required') setNotice({ msg: 'Please log in to continue.', kind: 'info' });
      ...
      <div className="min-h-screen flex items-center justify-center px-4 py-16 bg-background">
        ...
        >
          ...
          <div className="rounded-3xl border border-[#C9A84C]/30 p-8 md:p-10"
            ...
            {/* Auth notice banner */}
      {notice && (
        <div className={notice.kind === 'warn' ? 'flex items-start gap-2.5 p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 mb-6' : 'flex items-start gap-2.5 p-3.5 rounded-xl bg-blue-500/10 border border-blue-500/20 mb-6'}>
          <AlertCircle size={16} className={notice.kind === 'warn' ? 'text-amber-400 mt-0.5 shrink-0' : 'text-blue-400 mt-0.5 shrink-0'} />
          <p className={notice.kind === 'warn' ? 'text-sm text-amber-300' : 'text-sm text-blue-300'}>{notice.msg}</p>
        </div>
      )}
