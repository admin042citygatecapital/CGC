...
import {
  Shield, ArrowLeft, ShieldCheck, ShieldAlert,
  Smartphone, Monitor, Globe, Key,
  LogOut, Loader2, CheckCircle2, AlertTriangle, Lock,
  Eye, Fingerprint, Bell, MapPin,
  Copy, Laptop,
...
interface RawLoginEvent {
  id: string; ts: string; result: string; ip: string; device: string; browser: string; os: string; country: string;
}
interface RawSession {
  tokenPreview: string; ip: string; ua: string; createdAt: string; lastSeenAt: string; isCurrent: boolean;
}
...
interface LoginHistoryEntry {
  id: string; ip: string; device: string; createdAt: string; success: boolean;
}

const HIGH_SEVERITY_RESULTS = new Set(['failed', 'blocked', 'account_locked', 'totp_failed', 'otp_failed', 'status_denied']);

function toSecurityEvent(ev: RawLoginEvent): SecurityEvent {
  return {
    id: ev.id,
    description: `Login ${ev.result.replace(/_/g, ' ')} — ${ev.browser} on ${ev.os}`,
    ip: ev.ip,
    device: `${ev.browser} on ${ev.os}`,
    createdAt: ev.ts,
    severity: HIGH_SEVERITY_RESULTS.has(ev.result) ? 'high' : ev.result === 'success' ? 'low' : 'medium',
  };
}

function toActiveSession(s: RawSession): ActiveSession {
  return {
    id: s.tokenPreview,
    device: s.ua.length > 60 ? s.ua.slice(0, 57) + '...' : s.ua || 'Unknown device',
    ip: s.ip,
...
function toLoginHistoryEntry(ev: RawLoginEvent): LoginHistoryEntry {
  return {
    id: ev.id,
    ip: ev.ip,
    device: `${ev.browser} on ${ev.os}`,
...
export default function DashboardSecurityPage() {
  ...
  // 2FA state — this account's 2FA is email-OTP based (see users/2fa/setup),
  // not authenticator-app TOTP, so there's no secret/QR code to display.
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (evData?.events)     setEvents(evData.events.slice(0, 30).map(toSecurityEvent));
      if (sessData?.sessions) setSessions(sessData.sessions.map(toActiveSession));
      if (histData?.history)  setLoginHistory(histData.history.slice(0, 30).map(toLoginHistoryEntry));
  ...
  async function handleRevokeSession(tokenPreview: string) {
    if (!token || revokingId) return;
    setRevokingId(tokenPreview); setRevokeMsg(null);
    try {
      ...
      if (res.ok) { setSessions(prev => prev.filter(s => s.id !== tokenPreview)); setRevokeMsg('Session revoked.'); }
  ...
  async function handleSetup2FA() {
    ...
    try {
      ...
      if (res.ok) { setChallengeId(data.challengeId); setTotpMsg({ text: data.message ?? 'Verification code sent to your email.', ok: true }); }
      else setTotpMsg({ text: data.error ?? 'Failed to start 2FA setup.', ok: false });
    } catch { setTotpMsg({ text: 'Network error.', ok: false }); }
    finally { setTotpLoading(false); }
  }

  async function handleVerify2FA() {
    if (!token || !totpCode || !challengeId || totpLoading) return;
    setTotpLoading(true); setTotpMsg(null);
    try {
      const res = await fetch('/api/users/2fa/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ challengeId, otp: totpCode }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) { setTotpMsg({ text: '2FA enabled successfully!', ok: true }); setChallengeId(null); setTotpCode(''); }
  ...
  async function handleDisable2FA() {
    if (!token || !disablePassword || totpLoading) return;
    setTotpLoading(true); setTotpMsg(null);
    try {
      const res = await fetch('/api/users/2fa/setup', {
        method: 'POST',
      ...
      const data = await res.json().catch(() => ({}));
      if (res.ok) { setTotpMsg({ text: '2FA disabled.', ok: true }); setShowDisable(false); setDisablePassword(''); }
      else setTotpMsg({ text: data.error ?? 'Failed to disable 2FA.', ok: false });
    } catch { setTotpMsg({ text: 'Network error.', ok: false }); }
    finally { setTotpLoading(false); }
  }
                ...
                {events.length > 0 && (
                  <div className="flex flex-col gap-2">
                    ...
                    {events.slice(0, 5).map(ev => {
                      const EvIcon = eventIcon(ev.description);
                ...
                {sessions.length === 0 ? (
                  ...
                  <div key={sess.id} className="flex items-center gap-3 px-4 py-3.5 rounded-2xl border border-white/5 bg-white/[0.015]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-foreground/30">{sess.ip} · {timeAgo(sess.lastSeen)}</p>
                    </div>
                    {!sess.current && (
                  ...
                  <div key={entry.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl border border-white/5 bg-white/[0.015]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="flex items-center gap-1 text-[10px] text-foreground/20 font-mono"><MapPin size={8} />{entry.ip}</span>
                      </div>
                    </div>
                  ...
                  {!challengeId ? (
                    <div className="flex flex-col gap-4">
                      <p className="text-xs text-foreground/50 leading-relaxed">
                        We'll email a 6-digit verification code to your registered address each time you sign in once 2FA is enabled.
                      </p>
                      <button onClick={handleSetup2FA} disabled={totpLoading}
                        ...
                        {totpLoading ? <Loader2 size={14} className="animate-spin" /> : <Fingerprint size={14} />}
                        Enable Email 2FA
                      </button>

                      {!showDisable ? (
                        <button onClick={() => setShowDisable(true)} className="text-xs text-foreground/30 hover:text-foreground/60 transition-colors w-fit">
                          Already enabled? Turn off 2FA
                        </button>
                      ) : (
                        <div className="flex flex-col gap-2 p-3 rounded-xl bg-white/4 border border-white/8">
                          <p className="text-[10px] text-foreground/35">Confirm your password to disable 2FA</p>
                          <div className="flex gap-2">
                            <input
                              type="password" value={disablePassword} onChange={e => setDisablePassword(e.target.value)}
                              placeholder="Current password"
                              className="flex-1 bg-white/[0.04] border border-white/8 rounded-xl px-3 py-2 text-xs text-foreground/80 placeholder-foreground/20 focus:outline-none focus:border-primary/40"
                            />
                            <button onClick={handleDisable2FA} disabled={totpLoading || !disablePassword}
                              className="px-3 py-2 rounded-xl text-xs font-semibold text-red-400 border border-red-500/25 bg-red-500/8 hover:bg-red-500/12 transition-colors disabled:opacity-50">
                              Disable
                            </button>
                          </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-4">
                      <div className="flex items-center gap-2 p-3 rounded-xl bg-white/4 border border-white/8">
                        <Copy size={14} className="text-foreground/40 shrink-0" />
                        <p className="text-xs text-foreground/50">Check your email for the verification code.</p>
                      </div>

                      <div>
                        <p className="text-[10px] text-foreground/35 mb-1.5">Enter the 6-digit code from your email:</p>
                        <div className="flex gap-2">
                          <input
                      ...
                      <button onClick={() => { setChallengeId(null); setTotpCode(''); }}
