  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/users/profile', {
        ...
        body: JSON.stringify({ patch: { name, phone, country } }),
      });
  ...
  async function handleAvatarUpload(file: File) {
    ...
    reader.onload = async ev => {
      ...
      try {
        ...
        else setError(data.error ?? 'Failed to upload avatar');
      } catch {
        ...
        <main className="max-w-3xl mx-auto px-4 md:px-6 py-6">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <label className="relative cursor-pointer group shrink-0">
              <div className="w-16 h-16 rounded-2xl overflow-hidden flex items-center justify-center text-xl font-bold text-black"
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div>
                <label className="text-xs text-foreground/40 mb-1.5 block">Country</label>
                <div className="relative">
                  <MapPin size={13} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-foreground/25" />
                  <input className={`${inputCls} pl-9`} value={country} onChange={e => setCountry(e.target.value)} placeholder="Your country" />
                </div>
              </div>
