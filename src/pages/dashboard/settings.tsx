...
import {
  ...
  DollarSign, Key, Shield, Bell, ChevronRight,
  LogOut, Check,
} from 'lucide-react';
import { useCustomerAuth } from '@/lib/customerAuth';

interface SettingsState {
  privacyMode: boolean; darkMode: boolean; language: string; currency: string;
}
...
function Toggle({ checked, onChange, disabled }: { checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) {
  return (
    <button onClick={() => !disabled && onChange(!checked)} disabled={disabled}
      className="relative w-10 h-5.5 rounded-full transition-all shrink-0 disabled:opacity-40"
...
export default function SettingsPage() {
  ...
  const [settings, setSettings] = useState<SettingsState>({
    privacyMode: false, darkMode: true, language: 'English', currency: 'USD',
  });
  ...
  useEffect(() => {
    ...
    fetch('/api/users/notifications/preferences', { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then(data => { if (data?.preferences) setPushEnabled(!!data.preferences.push); })
      .catch(() => {})
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="divide-y divide-white/[0.04]">
              ...
              <div className="flex items-center gap-3 px-5 py-4">
                ...
                <Toggle checked={pushEnabled} onChange={updatePush} disabled={prefsLoading} />
              </div>
              {/* Language */}
              <div className="flex items-center gap-3 px-5 py-4">
                <div className="w-9 h-9 rounded-xl bg-white/5 border border-white/6 flex items-center justify-center shrink-0">
