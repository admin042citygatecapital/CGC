 ...
 * opens the Transfer Wizard modal. Transfers are always sent to a saved
 * beneficiary (see users/beneficiaries/*) — there is no ad-hoc recipient
 * entry, matching customerTransfer.ts's real contract
 * ({ currency, amount, beneficiaryId, note }). There is no per-transfer OTP
 * check server-side, so the wizard doesn't pretend to have one.
 */
...
import {
  ...
  CheckCircle, User, ShieldCheck, Copy, Plus,
} from 'lucide-react';
...
type WizardStep = 'recipient' | 'amount' | 'review' | 'confirmation';
...
function TransferWizard({
  onClose, balanceData, beneficiaries, token, initialBeneficiaryId,
}: {
  onClose: () => void;
  ...
  const [state, setState] = useState<WizardState>({
    beneficiaryId: initialBeneficiaryId ?? '', amount: '', note: '',
  });
  ...
  const steps: WizardStep[] = ['recipient', 'amount', 'review', 'confirmation'];
  const stepIdx = steps.indexOf(step);
  const selectedBeneficiary = beneficiaries.find(b => b.id === state.beneficiaryId);
  ...
  async function submitTransfer() {
    ...
    try {
      ...
      setFee(typeof data.fee === 'number' ? data.fee : null);
      setTxRef(data.transaction?.reference ?? data.transaction?.id ?? 'CGC-' + Date.now());
        ...
        <div className="px-6 py-5 min-h-[280px]">
          ...
          {step === 'recipient' && (
            <div className="flex flex-col gap-4">
              <div>
                <p className="text-sm font-semibold text-foreground mb-1">Select a recipient</p>
                <p className="text-xs text-foreground/35 mb-4">Choose a saved beneficiary to send to</p>
              </div>
              {beneficiaries.length === 0 ? (
                <div className="flex flex-col items-center gap-3 py-8 text-center">
                  <p className="text-xs text-foreground/30">You have no saved beneficiaries yet.</p>
                  <Link to="/dashboard/beneficiaries" onClick={onClose}
                    ...
                    <Plus size={13} /> Add a Beneficiary
                  </Link>
                </div>
              ) : (
                <div className="flex flex-col gap-2 max-h-72 overflow-y-auto">
                  {beneficiaries.map(b => (
                    <button key={b.id} onClick={() => update({ beneficiaryId: b.id })}
                      className="flex items-center gap-3 p-3.5 rounded-xl border transition-all text-left"
                      style={{
                        background: state.beneficiaryId === b.id ? 'rgba(201,168,76,0.08)' : 'rgba(255,255,255,0.02)',
                        borderColor: state.beneficiaryId === b.id ? 'rgba(201,168,76,0.35)' : 'rgba(255,255,255,0.07)',
                      }}>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-foreground truncate">{b.nickname || b.name}</p>
                        <p className="text-xs text-foreground/35 truncate">
                          {b.type === 'bank' ? `${b.bankName ?? ''} ···${(b.accountNumber ?? '').slice(-4)}` : `${b.asset} · ${(b.walletAddress ?? '').slice(0, 6)}···${(b.walletAddress ?? '').slice(-4)}`}
                        </p>
                      </div>
                      {state.beneficiaryId === b.id && <Check size={14} className="text-primary shrink-0" />}
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-xs text-foreground/35 mb-4">
                  Available: {balanceData ? `${fmt(balanceData.balance, currency)}` : '—'}
                </p>
              </div>
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-foreground/40 text-sm font-semibold">{currency}</span>
                <input
          ...
          {step === 'review' && (
            <div className="flex flex-col gap-3">
              ...
              {[
                { label: 'To', value: selectedBeneficiary?.nickname || selectedBeneficiary?.name || '—' },
                { label: 'Amount', value: `${state.amount || '0'} ${currency}` },
          ...
          {step === 'confirmation' && (
            <div className="flex flex-col items-center gap-4 py-4 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-foreground/35">Your transfer is pending admin approval{fee !== null && fee > 0 ? ` (fee: ${fmt(fee, currency)})` : ''}.</p>
              </div>
              {txRef && (
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-white/6">
            ...
            {step === 'review' ? (
              ...
              <button
                onClick={next}
                disabled={
                  (step === 'recipient' && !state.beneficiaryId) ||
...
export default function TransfersPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch('/api/users/beneficiaries', { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : null),
    ]).then(([txData, balData, benData]) => {
      if (txData?.transactions) setAllTx(txData.transactions);
      if (balData) setBalanceData({ balance: Number(balData.balance ?? 0), currency: balData.currency ?? 'USD' });
  ...
  useEffect(() => {
    if (searchParams.get('action') === 'new' || searchParams.get('beneficiary')) {
      setInitialBeneficiaryId(searchParams.get('beneficiary') ?? undefined);
      setWizardOpen(true);
    }
  }, [searchParams]);
  ...
  function closeWizard() {
    ...
    // Re-fetch after closing so a just-submitted transfer shows up immediately
    if (token) {
      fetch('/api/users/transactions?limit=200', { headers: { Authorization: `Bearer ${token}` } })
        .then(r => r.ok ? r.json() : null)
        .then(d => { if (d?.transactions) setAllTx(d.transactions); })
        .catch(() => {});
    }
  }
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {wizardOpen && (
          <TransferWizard onClose={closeWizard} balanceData={balanceData} beneficiaries={beneficiaries} token={token} initialBeneficiaryId={initialBeneficiaryId} />
        )}
      </AnimatePresence>
