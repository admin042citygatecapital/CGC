...
import {
  ArrowLeft, Zap,
  Loader2, CheckCircle, AlertTriangle, ShieldAlert, Target,
  ChevronDown, ChevronUp, BarChart2, BookOpen, Activity,
  DollarSign, Info, X,
} from 'lucide-react';
import { useCustomerAuth } from '@/lib/customerAuth';
import { useTicker, useOrderBook } from '@/hooks/useMarketData';
...
// ─────────────────────────────────────────────────────────────────────────────
// Order book — real depth from the market-data provider registry
// ─────────────────────────────────────────────────────────────────────────────

function withRunningTotal(levels: { price: number; quantity: number }[]) {
  let total = 0;
  return levels.map(l => { total += l.quantity; return { price: l.price, qty: l.quantity, total }; });
}
...
function OrderBook({ symbol, midPrice, onPriceClick }: { symbol: string; midPrice: number; onPriceClick: (p: number) => void }) {
  const { orderBook, loading } = useOrderBook(symbol, 8, 'crypto', 3000);
  const asks = withRunningTotal([...(orderBook?.asks ?? [])].reverse()).reverse();
  const bids = withRunningTotal(orderBook?.bids ?? []);
  ...
  if (loading && !orderBook) {
    return (
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden flex items-center justify-center py-16">
